...
namespace CustomActivatableEquipment
{
    ...
    public static class ComponentDefHelper
    {
        ...
        {
            object result = null;
            if (source is WeaponDef) { result = new WeaponDynamicDef(); }
            else
            if (source is UpgradeDef) { result = new UpgradeDynamicDef(); }
            else
            if (source is StatUpgradeDef) { result = new StatUpgradeDynamicDef(); }
            else
            if (source is JumpJetDef) { result = new JumpJetDynamicDef(); }
            else
            if (source is HeatSinkDef) { result = new HeatSinkDynamicDef(); }
            else
            if (source is AmmunitionBoxDef) { result = new AmmunitionBoxDynamicDef(); }
            else
            if (source is MechComponentDef) { result = new MechComponentDynamicDef(); }
            else
            {
        ...
        public static object DeepCopy(object source)
        {
            try
            {
                ...
                if ((result is WeaponDef dest_WeaponDef) && (source is WeaponDef src_WeaponDef)) { CopyFromTo_WeaponDef(src_WeaponDef, dest_WeaponDef); }
                else
                if ((result is UpgradeDef dest_UpgradeDef) && (source is UpgradeDef src_UpgradeDef)) { CopyFromTo_UpgradeDef(src_UpgradeDef, dest_UpgradeDef); }
                else
                if ((result is StatUpgradeDef dest_StatUpgradeDef) && (source is StatUpgradeDef src_StatUpgradeDef)) { CopyFromTo_StatUpgradeDef(src_StatUpgradeDef, dest_StatUpgradeDef); }
                else
                if ((result is JumpJetDef dest_JumpJetDef) && (source is JumpJetDef src_JumpJetDef)) { CopyFromTo_JumpJetDef(src_JumpJetDef, dest_JumpJetDef); }
                else
                if ((result is HeatSinkDef dest_HeatSinkDef) && (source is HeatSinkDef src_HeatSinkDef)) { CopyFromTo_HeatSinkDef(src_HeatSinkDef, dest_HeatSinkDef); }
                else
                if ((result is AmmunitionBoxDef dest_AmmunitionBoxDef) && (source is AmmunitionBoxDef src_AmmunitionBoxDef)) { CopyFromTo_AmmunitionBoxDef(src_AmmunitionBoxDef, dest_AmmunitionBoxDef); }
                else
                if ((result is MechComponentDef dest_MechComponentDef) && (source is MechComponentDef src_MechComponentDef)) { CopyFromTo_MechComponentDef(src_MechComponentDef, dest_MechComponentDef); }
                else
                {
                    result = source;
                }
