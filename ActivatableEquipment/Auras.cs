...
namespace CustomActivatableEquipment
{
    ...
    public static partial class CAEAuraHelper
    {
        ...
        {
            if (actorAuraBodies.ContainsKey(unit) == false) { actorAuraBodies.Add(unit, body); }
            else
            {
    ...
    public class AuraActorBody : MonoBehaviour
    {
        ...
        public void ShowAddFloatie(AuraBubble aura)
        {
            try
            {
                ...
                if (owner.GUID == auraOwner.GUID) { isAlly = true; }
                else
                  if (owner.TeamId == auraOwner.TeamId) { isAlly = true; }
                else
                {
                    if (owner.team == null)
                    {
    ...
    public class AuraBubble : MonoBehaviour
    {
        ...
        public void UpdateRadius(bool now = false)
        {
            ...
            if (source != null)
            {
                ...
                {
                    if ((source.isActive() == false) && (Def.State == AuraState.Online)) { radius = 0.1f; goto apply_radius; }
                    else
        ...
        public void Update()
        {
            try
            {
                ...
                if (sdelta >= delta) { collider.radius = FRadius; Speed = 0f; }
                else
                {
